...
func buildVmessUser(userInfo *[]api.UserInfo, serverAlterID int) (users []*protocol.User) {
	...
	for i, user := range *userInfo {
		...
		users[i] = &protocol.User{
			Level:   0,
			Email:   fmt.Sprintf("%s|%d", user.Email, user.UID), // Email: InboundTag|email|uid
...
func buildSSUser(userInfo *[]api.UserInfo, method string) (users []*protocol.User) {
...
func buildSSPluginUser(userInfo *[]api.UserInfo, method string) (users []*protocol.User) {
	users = make([]*protocol.User, 0)
	cypherMethod := cipherFromString(method)
	for _, user := range *userInfo {
		ssAccount := &shadowsocks.Account{
