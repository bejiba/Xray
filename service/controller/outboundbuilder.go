...
func OutRelayboundBuilder(config *Config, nodeInfo *api.RelayNodeInfo ,UUID string, Email string, Passwd string, UID int) (*core.OutboundHandlerConfig, error) {
		outboundDetourConfig := &conf.OutboundDetourConfig{}
		...
		if nodeInfo.NodeType == "Vless" {
				...
				VlessUsers := buildRVlessUser(fmt.Sprintf("%s|%d%d", nodeInfo.NodeType, nodeInfo.Port, nodeInfo.NodeID,), UUID, Email)
				...
				users := buildRVmessUser(fmt.Sprintf("%s|%d|%d", nodeInfo.NodeType, nodeInfo.Port, nodeInfo.NodeID), UUID, Email, nodeInfo.AlterID)
				...
				if nodeInfo.TLSType == "xtls" {
					proxySetting = struct {
						...
						Servers: []*TrojanServerTarget{&TrojanServerTarget{
								...
								Port:     uint16(nodeInfo.Port),
								Password: UUID,
								Email:    fmt.Sprintf("%s|%s", Email, UUID),
								Level:    0,
		...
		outboundDetourConfig.Tag = fmt.Sprintf("Relay_%d", UID)
		if config.SendIP != "" {
			ipAddress := net.ParseAddress(config.SendIP)
			outboundDetourConfig.SendThrough = &conf.Address{ipAddress}
		}
...
func buildRVlessUser(tag string, UUID string, Email string)  *protocol.User {
		...
		return &protocol.User{
			Level:   0,
			Email:   fmt.Sprintf("%s|%s", tag, Email, UUID),
