...
func (c *Controller) Start() error {
	...
	if c.nodeInfo.Relay == 1 {	
		...
		err = c.RelayTag(newRelayNodeInfo, userInfo)
		if err != nil {
				log.Panic(err)
				return err
		}
...
func (c *Controller) nodeInfoMonitor() (err error) {
	...
	if !reflect.DeepEqual(c.nodeInfo, newNodeInfo) {
		...
		if newNodeInfo.Relay == 1 {
			...
			err = c.RelayTag(newRelayNodeInfo, newUserInfo)
			if err != nil {
					log.Panic(err)
					return err
			}
...
func (c *Controller) removeRelayTag(tag string) (err error) {
...
func (c *Controller) RelayTag(newRelayNodeInfo *api.RelayNodeInfo, userInfo *[]api.UserInfo) (err error) {
	if newRelayNodeInfo.NodeType != "Shadowsocks-Plugin" {
		for _, user := range *userInfo {
			if user.RelayUser == 1 {
				if c.Rtag {
					err := c.removeRelayTag(fmt.Sprintf("Relay_%d", user.UID))
					if err != nil {
						return err
					}
				}
				outRelayBoundConfig, err := OutRelayboundBuilder(c.config, newRelayNodeInfo, user.UUID, user.Email, user.Passwd, user.UID)
				if err != nil {
					return err
				}
